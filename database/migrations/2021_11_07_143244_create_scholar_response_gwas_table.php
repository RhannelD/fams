...
class CreateScholarResponseGwasTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('scholar_response_gwas', function (Blueprint $table) {
            $table->id();
            $table->foreignId('response_id');
            $table->foreignId('item_id');
            $table->double('gwa', 3, 2);
            $table->timestamps();
    ...
    {
        Schema::table('scholar_response_gwas', function (Blueprint $table) {
            $table->dropForeign(['response_id']);
            $table->dropForeign(['item_id']);
            $table->dropColumn(['response_id', 'item_id']);
        });
        
        Schema::dropIfExists('scholar_response_gwas');
    }
}
